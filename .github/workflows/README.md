...
发布完成后，工作流会对实际镜像执行 amd64/arm64 依赖导入检查，并验证两个镜像仓库的架构、`org.opencontainers.image.revision` 和索引摘要完全一致。GHCR 中当前有效镜像引用的 amd64/arm64 清单、SBOM 和来源证明属于镜像组成部分，会予以保留。部署者可以继续使用 `aethersailor/rule-bot:latest`，也可以改用 `ghcr.io/aethersailor/rule-bot:latest`；现有 Compose 和更新方式不变。
