...
class Config:
    ...
    def __init__(self):
        ...
        # Rule-Bot Client HTTP ingress is opt-in.
        self.RULE_BOT_CLIENT_PRIVATE_API_ENABLED = self._parse_bool_env(
            "RULE_BOT_CLIENT_PRIVATE_API_ENABLED", False
        )
        self.RULE_BOT_CLIENT_PRIVATE_API_HOST = os.getenv(
            "RULE_BOT_CLIENT_PRIVATE_API_HOST", "0.0.0.0"
        ).strip()
        self.RULE_BOT_CLIENT_PRIVATE_API_PORT = self._parse_int_env(
            "RULE_BOT_CLIENT_PRIVATE_API_PORT", 8765, min_value=1, max_value=65535
        )
        self.RULE_BOT_CLIENT_PRIVATE_API_PATH = self._parse_api_path(
            "RULE_BOT_CLIENT_PRIVATE_API_PATH"
        )
        self.RULE_BOT_CLIENT_PRIVATE_API_TOKEN = self._read_secret(
            "RULE_BOT_CLIENT_PRIVATE_API_TOKEN"
        )
        self.RULE_BOT_CLIENT_PRIVATE_API_RATE_LIMIT_PER_HOUR = self._parse_int_env(
            "RULE_BOT_CLIENT_PRIVATE_API_RATE_LIMIT_PER_HOUR", 1000, min_value=1
        )
        self.RULE_BOT_CLIENT_PRIVATE_API_TLS_CERT_FILE = os.getenv(
            "RULE_BOT_CLIENT_PRIVATE_API_TLS_CERT_FILE", ""
        ).strip()
        self.RULE_BOT_CLIENT_PRIVATE_API_TLS_KEY_FILE = os.getenv(
            "RULE_BOT_CLIENT_PRIVATE_API_TLS_KEY_FILE", ""
        ).strip()

        self.RULE_BOT_CLIENT_COMMUNITY_API_ENABLED = self._parse_bool_env(
            "RULE_BOT_CLIENT_COMMUNITY_API_ENABLED", False
        )
        self.RULE_BOT_CLIENT_COMMUNITY_API_HOST = os.getenv(
            "RULE_BOT_CLIENT_COMMUNITY_API_HOST", "0.0.0.0"
        ).strip()
        self.RULE_BOT_CLIENT_COMMUNITY_API_PORT = self._parse_int_env(
            "RULE_BOT_CLIENT_COMMUNITY_API_PORT", 7654, min_value=1, max_value=65535
        )
        self.RULE_BOT_CLIENT_COMMUNITY_API_PATH = self._parse_api_path(
            "RULE_BOT_CLIENT_COMMUNITY_API_PATH"
        )
        self.RULE_BOT_CLIENT_COMMUNITY_API_BASE_URL = os.getenv(
            "RULE_BOT_CLIENT_COMMUNITY_API_BASE_URL", ""
        ).strip().rstrip("/")
        self.RULE_BOT_CLIENT_COMMUNITY_API_RATE_LIMIT_PER_HOUR = self._parse_int_env(
            "RULE_BOT_CLIENT_COMMUNITY_API_RATE_LIMIT_PER_HOUR", 50, min_value=1
        )
        self.RULE_BOT_CLIENT_COMMUNITY_API_TLS_CERT_FILE = os.getenv(
            "RULE_BOT_CLIENT_COMMUNITY_API_TLS_CERT_FILE", ""
        ).strip()
        self.RULE_BOT_CLIENT_COMMUNITY_API_TLS_KEY_FILE = os.getenv(
            "RULE_BOT_CLIENT_COMMUNITY_API_TLS_KEY_FILE", ""
        ).strip()
        self.RULE_BOT_CLIENT_COMMUNITY_TOKEN_SIGNING_KEY = self._read_secret(
            "RULE_BOT_CLIENT_COMMUNITY_TOKEN_SIGNING_KEY"
        )
        self.RULE_BOT_CLIENT_COMMUNITY_TOKEN_TTL_DAYS = self._parse_int_env(
            "RULE_BOT_CLIENT_COMMUNITY_TOKEN_TTL_DAYS", 90, min_value=1, max_value=365
        )
        token_database = os.getenv("RULE_BOT_CLIENT_COMMUNITY_TOKEN_DATABASE", "").strip()
        self.RULE_BOT_CLIENT_COMMUNITY_TOKEN_DATABASE = Path(token_database) if token_database else None
        ...
        self._validate_rule_bot_client_config()
    ...
    def _validate_rule_bot_client_config(self) -> None:
        if self.RULE_BOT_CLIENT_PRIVATE_API_ENABLED:
            if not self.RULE_BOT_CLIENT_PRIVATE_API_PATH:
                raise ValueError("RULE_BOT_CLIENT_PRIVATE_API_PATH is required")
            if len(self.RULE_BOT_CLIENT_PRIVATE_API_TOKEN) < 32:
                raise ValueError("RULE_BOT_CLIENT_PRIVATE_API_TOKEN must be at least 32 characters")
        if self.RULE_BOT_CLIENT_COMMUNITY_API_ENABLED:
            if not self.RULE_BOT_CLIENT_COMMUNITY_API_PATH:
                raise ValueError("RULE_BOT_CLIENT_COMMUNITY_API_PATH is required")
            if len(self.RULE_BOT_CLIENT_COMMUNITY_TOKEN_SIGNING_KEY) < 32:
                raise ValueError("RULE_BOT_CLIENT_COMMUNITY_TOKEN_SIGNING_KEY must be at least 32 characters")
            parsed_base_url = urlparse(self.RULE_BOT_CLIENT_COMMUNITY_API_BASE_URL)
            if (
                ...
                raise ValueError("RULE_BOT_CLIENT_COMMUNITY_API_BASE_URL must be an HTTP(S) origin")
            if not self.GROUP_CHECK_ENABLED:
                raise ValueError(
                    "Rule-Bot Client Community API requires group membership verification"
                )
        if (
            self.RULE_BOT_CLIENT_PRIVATE_API_ENABLED
            and self.RULE_BOT_CLIENT_COMMUNITY_API_ENABLED
            and self.RULE_BOT_CLIENT_PRIVATE_API_HOST == self.RULE_BOT_CLIENT_COMMUNITY_API_HOST
            and self.RULE_BOT_CLIENT_PRIVATE_API_PORT == self.RULE_BOT_CLIENT_COMMUNITY_API_PORT
        ):
            raise ValueError(
                "Rule-Bot Client private and community APIs cannot share one listener"
            )
        for prefix in ("RULE_BOT_CLIENT_PRIVATE_API", "RULE_BOT_CLIENT_COMMUNITY_API"):
